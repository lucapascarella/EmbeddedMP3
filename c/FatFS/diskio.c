...
void power_on(void) {
    ...
    SpiChnOpen(SPI_CHANNEL, SPI_OPEN_MSTEN | SPI_OPEN_CKP_HIGH | SPI_OPEN_SMP_END | SPI_OPEN_MODE8, GetPeripheralClock() / 400000);
...
int rcvr_datablock(/* 1:OK, 0:Failed */
        ...
        volatile UINT btr /* Byte count (must be multiple of 4) */
        ) {
    BYTE token;
    ...
    //SPICON1bits.ENHBUF = 1;
    //
    //do {
    //if (SPISTATbits.SPIRBE == 0) {
    //    dummy = SPIBUF;
    //    transferWordToByte(dummy, buff);
    //}
...
int xmit_datablock(/* 1:OK, 0:Failed */
    ...
    if (token != 0xFD) { /* Not StopTran token */
        ...
        //	i = k = 0;
        //	do {
        //	    if (SPISTATbits.SPIRBF == 0 && i < 512) {
        //		SPIBUF = (buff[i] << 24) | (buff[i + 1] << 16) | (buff[i + 2]) << 8 | (buff[i + 3]);
        //		i += 4;
        //	    }
        //	    if (SPISTATbits.SPIRBE == 0) {
        //		dummy = SPIBUF;
        ...
        i = 0;
        SPIBUF = buff[i] << 24 | buff[i + 1] << 16 | buff[i + 2] << 8 | buff[i + 3];
        while ((i += 4) < 512) { /* Xmit the 512 byte data block to the MMC */
            tmp = buff[i] << 24 | buff[i + 1] << 16 | buff[i + 2] << 8 | buff[i + 3];
            while (!SPISTATbits.SPIRBF);
            dummy = SPIBUF;
            SPIBUF = tmp;
        }
