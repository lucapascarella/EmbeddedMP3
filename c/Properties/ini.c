...
int ini_parse_file(FIL* file, int (*handler)(void*, const char*, const char*, const char*), void* user) {
...
int ini_parse(const char* filename, int (*handler)(void*, const char*, const char*, const char*), void* user) {
    ...
    fres = f_open(fp, filename, FA_READ);
    if (fres != FR_OK)
        return -1;
    error = ini_parse_file(fp, handler, user);
